...
# uvがインストールされているか確認（featuresでインストールされることを前提）
if ! command -v uv &> /dev/null
then
    echo "Critical Error: uv command not found. Codespace setup failed."
    exit 1
fi
...
# `PATH` を適切に設定することで、`./.venv/bin/uv` のように明示的なパス指定を避けることも可能だが、
# Codespacesの起動シーケンスの複雑さを考慮し、ここでは明示的なパスを維持
